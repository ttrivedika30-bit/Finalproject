...
exports.errorResponseStreamConverter = errorResponseStreamConverter;
const { pipeline } = require('node:stream/promises');
const { Writable, Readable } = require('node:stream');
function errorResponseInterceptor(axiosError) {
    if (axiosError.response && // must have a response
        ...
        (axiosError.config.responseType === 'json' ||
            ...
            (axiosError.config.responseType === 'stream' &&
                axiosError.response.headers['content-type'].startsWith('application/json') &&
        ...
        if (axiosError.response.data.errors) {
            ...
            const trace = axiosError.response.headers['x-request-id'] ||
 ...
 */
async function errorResponseStreamConverter(axiosError) {
    if (axiosError.response && // must have a response
        axiosError.response.status >= 400 &&
        // must have data:
        axiosError.response.data &&
        ...
        axiosError.response.data instanceof Readable) {
        let data = '';
        await pipeline(axiosError.response.data, new Writable({
            write: (c, e, cb) => {
                data += c;
                cb();
            },
...
//# sourceMappingURL=errorResponseInterceptor.js.map